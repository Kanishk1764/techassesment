...
import { X, Briefcase, Plus, AlertCircle, Sparkles, Sliders, Upload, CheckCircle2, Wand2 } from 'lucide-react';
...
interface CreateJobModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (jobData: CreateJobDto) => Promise<void>;
}
...
export const CreateJobModal: React.FC<CreateJobModalProps> = ({
  ...
  // Smooth Individual Weightage Sliders (min 0, max 100, step 1)
  const [skillsWeight, setSkillsWeight] = useState<number>(50);
  const [experienceWeight, setExperienceWeight] = useState<number>(35);
  const [educationWeight, setEducationWeight] = useState<number>(15);

  // AI JD Enhancer state
  ...
  // Always reset fields when opening the modal so previous job creation data is never retained
  useEffect(() => {
    if (isOpen) {
      setTitle('');
      setDescription('');
  ...
  // Helper to normalize weights to exactly 100%
  ...
  // Extract skills with AI from description
  ...
  // AI JD Enhancement from raw notes or uploaded file
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (skillsList.length === 0) {
      setError('Please add at least one required skill (type manually or click "Extract Skills with AI").');
      return;
    }
    ...
    if (totalWeight !== 100) {
      setError(`Evaluation weights must total 100% (currently ${totalWeight}%). Click 'Auto-Balance to 100%' or adjust the bars.`);
      return;
    }
      ...
      <div className="w-full max-w-3xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/80 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Create New Job Position</h2>
              <p className="text-xs text-slate-400">Define requirements, screening weights, and AI criteria</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5 flex-1">
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-r from-sky-950/40 via-indigo-950/40 to-slate-950 border border-sky-900/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <div className="text-[11px] text-slate-400 mt-0.5">
                Upload a raw JD document (.pdf, .docx, .txt) or paste rough notes to auto-format.
              </div>
            </div>
              ...
              >
                {isEnhancing ? (
                  ...
                  <>
                    ...
                    <span>Polish Notes with AI</span>
                  </>
                )}
              </button>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[11px] text-slate-500">Supports full markdown (# Headers, * Lists, **Bold**)</span>
            </div>
            <textarea
              required
              rows={6}
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-sky-500 font-mono text-xs resize-y"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <span className="text-[11px] text-slate-500 mt-1 block">
                Strict experience gating will apply if candidate falls below this threshold.
              </span>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-300">
                  Add Required Skills <span className="text-sky-400">*</span>
                </label>
                <button
                  type="button"
            ...
            <div className="flex flex-wrap gap-1.5 min-h-[42px] p-2.5 rounded-xl bg-slate-950/60 border border-slate-800/80 items-center">
              ...
              {skillsList.length === 0 && (
                <span className="text-xs text-slate-500 italic">
                  No skills added yet. Add custom skills above or click <strong className="text-sky-400">"Extract with AI"</strong>.
                </span>
              )}
            </div>
          </div>
            ...
            <div className="space-y-4 pt-1">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-slate-500 text-[11px]">Core skill overlap ratio</span>
                    <input
                      type="number"
                      min={0}
                      max={100}
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-slate-500 text-[11px]">Timeline & tenure verification</span>
                    <input
                      type="number"
                      min={0}
                      max={100}
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-slate-500 text-[11px]">Background & degree alignment</span>
                    <input
                      type="number"
                      min={0}
                      max={100}
